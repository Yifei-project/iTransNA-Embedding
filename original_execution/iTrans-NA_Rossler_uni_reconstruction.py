...
class net_configs:  # univariate
    def __init__(self) -> None:
        self.seq_len = 50             # W, also equal to 'time_window' below
        self.pred_len = 40            # L_{pred}
        self.d_model = 32             # the encoder dimension
        self.n_heads = 2              # attention head number
...
if __name__ == "__main__":
    ...
    for id in range(1,101):
        ...
        h5f = h5.File('./results/iTrans-NA/Rossler/rossler_uni_no'+str(id)+'.h5', 'w')
    ...
    for id in range(1,101):
        ...
        # save the final reconstruction result
        h5f = h5.File('./results/iTrans/Rossler/rossler_uni_no'+str(id)+'.h5', 'w')
        dset = h5f.create_dataset("Embedding", Recon.shape, dtype='f')
        dset[...] = Recon
        h5f.close()
